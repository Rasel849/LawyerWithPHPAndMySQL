...
{
    session_start();
    $_SESSION['variable_name_24']=false;
    $email=$_POST["email"];
    $password=$_POST["password"];
    $flag=1;
    $query=mysqli_query($connect,"select * from user_registation");
    while($row=mysqli_fetch_array($query))
    {
        ...
        if(  $email==$row['email'] )
        {
            ...
            {
                session_start();
                $_SESSION['variable_name_1'] = $row['username'];
                $_SESSION['variable_name_2'] = $row['email'];
                $flag=0;
                header('Location:../../../Components/page/HomePage.php');
                break;
            }
            else{
                $emailvalue=$email;
    ...
    <div class="container">
        ...
        <div class="fillDetail">
            ...
            <div class="createAccount">
                ...
                <p>If You have no Account ? <a href="./registation.php" style="text-decoration: none; "> sign up !</a></p>
            </div>
        </div>
    </div>
